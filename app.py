...
def sales():
    ...
    sale = {
        ...
        'cashierName': next((u['name'] for u in load_json('users.json') if u['id'] == request.user.get('id')), 'Unknown'),
        'createdAt': datetime.now().isoformat()
    }
...
def stats():
    ...
    # Cashier activity stats
    cashier_stats = {}
    for sale in sales:
        cashier_id = sale.get('cashierId')
        if cashier_id:
            if cashier_id not in cashier_stats:
                cashier_stats[cashier_id] = {'sales': 0, 'total': 0}
            cashier_stats[cashier_id]['sales'] += 1
            cashier_stats[cashier_id]['total'] += sale.get('total', 0)
...
def demo_requests():
    if request.method == 'GET':
        ...
        demo_requests = load_json('demo_requests.json')
        return jsonify(demo_requests)
    ...
    save_json('demo_requests.json', demo_requests)
    return jsonify(demo_request), 201

@app.route('/api/demo-requests/<int:id>/approve', methods=['POST'])
@token_required
def approve_demo(id):
    if request.user.get('role') != 'admin':
        return jsonify({'error': 'Admin access required'}), 403
    ...
    demo_requests = load_json('demo_requests.json')
    demo_request = next((r for r in demo_requests if r['id'] == id), None)
    if not demo_request:
        return jsonify({'error': 'Demo request not found'}), 404
    
    demo_request['status'] = 'approved'
    demo_request['approvedAt'] = datetime.now().isoformat()
    save_json('demo_requests.json', demo_requests)
...
def signup_with_payment():
    """Create user account with payment in one step"""
    data = request.json
    users = load_json('users.json')
    payments = load_json('payments.json')
    ...
    payment = {
        ...
        'fullName': data['name'],
        'amount': data['amount'],
        'method': data.get('paymentMethod', 'mpesa'),
    ...
    """Handle payments from non-authenticated users"""
    data = request.json
    payments = load_json('payments.json')
...
@app.route('/api/payments/<int:id>/approve', methods=['POST'])
@token_required
def approve_payment(id):
    ...
    payments = load_json('payments.json')
    payment = next((p for p in payments if p['id'] == id), None)
    if not payment:
    ...
    # Check if user already exists
    existing_user = next((u for u in users if u['email'] == payment['email']), None)
    if existing_user:
        return jsonify({'error': 'User already exists'}), 400
    ...
    user = {
        ...
        'role': 'admin' if payment['plan'] == 'ultra' else 'cashier',
        'plan': payment['plan'],
    ...
    payment['approvedAt'] = datetime.now().isoformat()
    save_json('payments.json', payments)
    
    return jsonify({'payment': payment, 'user': {k: v for k, v in user.items() if k != 'password'}})
