                ...
                return (
                  <div key={idx} className="border border-gray-200 rounded-lg p-4 space-y-2 bg-white">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-sm text-gray-900">{record.cashierName || record.userName}</span>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        record.status === 'clocked_in' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <span className="font-medium">{clockInTime.toLocaleDateString()}</span>
                      </div>
                      <div>
                        <span className="text-xs text-gray-500 block">Duration</span>
                        <span className="badge badge-success">{record.duration ? formatDuration(record.duration) : '-'}</span>
                      </div>
                      <div>
                        <span className="text-xs text-gray-500 block">Clock In</span>
                        <span className="font-medium">{clockInTime.toLocaleTimeString()}</span>
                      </div>
                      <div>
                        <span className="text-xs text-gray-500 block">Clock Out</span>
                        <span className="font-medium">{clockOutTime ? clockOutTime.toLocaleTimeString() : '-'}</span>
                      </div>
                    </div>
                  </div>
                );
